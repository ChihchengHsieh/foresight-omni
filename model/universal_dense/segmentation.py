...
class DETRsegm(nn.Module):
    def __init__(self, detr, freeze_detr=False):
        ...
        dim, nheads = detr.transformer.dim, detr.transformer.nhead
        self.bbox_attention = MHAttentionMap(dim, dim, nheads, dropout=0.0)
        self.mask_head = MaskHeadSmallConv(dim + nheads, [1024, 512, 256], dim)
...
class MHAttentionMap(nn.Module):
    ...
    def __init__(self, query_dim, dim, num_heads, dropout=0.0, bias=True):
        super().__init__()
        self.num_heads = num_heads
        self.dim = dim
        self.dropout = nn.Dropout(dropout)

        self.q_linear = nn.Linear(query_dim, dim, bias=bias)
        self.k_linear = nn.Linear(query_dim, dim, bias=bias)

        nn.init.zeros_(self.k_linear.bias)
        nn.init.zeros_(self.q_linear.bias)
        nn.init.xavier_uniform_(self.k_linear.weight)
        nn.init.xavier_uniform_(self.q_linear.weight)
        self.normalize_fact = float(dim / self.num_heads) ** -0.5
    ...
    def forward(self, q, k, mask: Optional[Tensor] = None):
        ...
        qh = q.view(q.shape[0], q.shape[1], self.num_heads, self.dim // self.num_heads)
        kh = k.view(
            k.shape[0],
            self.num_heads,
